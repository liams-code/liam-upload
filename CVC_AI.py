...
# 자동매매 시작 , CVC 5000원 이상으로 바꿈 , seconds을 hours=2로 바꿔서 7시에 매도하게 함.
while True:
    try:
        ...
        avg_buy_price = get_avg_buy_price("KRW-CVC")
        if current_price > (avg_buy_price*1.12) or current_price < (avg_buy_price*0.94):
            cvc = get_balance("CVC")
            if cvc > 14:
                upbit.sell_market_order("KRW-CVC", cvc*0.9995)

        if start_time < now < end_time - datetime.timedelta(hours=1):
            target_price = get_target_price("KRW-CVC", 0.4)
            current_price = get_current_price("KRW-CVC")
